...
Rodando o código acima sem a última linha estar comentada, obteremos um erro vindo do último cruzamento, um left join entre *ocorrencia* e *recomendacao*:
...
Bom, dado que não há uma chave comum entre as tabelas que não estejam duplicadas, não é possível fazer um join direto entre elas. No entanto, como nossas tabelas não possuem nenhum campo de valor que poderíamos agregar (exceto 3 colunas que contém o nome "total", mas que podem ser recalculadas usando as variáveis de origem), nós conseguimos fazer algo ainda que nos ajude a colocar tudo numa tabela só, usando cruzamento cartesiano (*cartesian join*).
...
E sucesso! *Wheeeew.* Agora que temos uma tabela com todas as informações que precisamos, podemos criar algumas visões para tirar alguns insights ou análises interessantes sobre :)
