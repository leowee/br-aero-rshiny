...
Primeira coisa, vamos dar um head para ter um panorama geral da tabela central, *ocorrencia* - especialmente das chaves. Nunca se sabe se algo deu errado.
...
Aparentemente as chaves de alguma das duas tabelas não são únicas. Como a tabela central, *ocorrencia*, cruzou com as outras 3 tabelas, vamos dar uma olhadinha na tabela *recomendacao*:

```{r Panorama geral da tabela recomendacao}
str(recomendacao)
```
...
Bom, não existeu metadados. Vamos na marra então!

Como a chave *codigo_ocorrencia4* repete duas vezes, vamos pegar o primeiro elemento da coluna como exemplo e ver como ela se comporta.

```{r Exemplo da tabela recomendacao 1}
recomendacao[codigo_ocorrencia4 == 39115]
```

```{r Exemplo da tabela recomendacao 2, echo=FALSE}
recomendacao[codigo_ocorrencia4 == 39115, recomendacao_conteudo]
```
Hmm... Aparentemente existem **múltiplas recomendações para uma mesma ocorrência**. Faz sentido o nosso left join não ter funcionado plenamente. No entanto, isso é facilmente consertado trocando o join de **left join** para **right join** dado que sabemos que os valores da coluna *codigo_ocorrencia4* existem apenas uma única vez na Tabela Central, *ocorrencias*.

Vamos tentar fazer o código novamente trocando o último cruzamento de left join para right join, e ver se dará certo.

> cruzamento1 <- ocorrencia[ocorrencia_tipo, on = "codigo_ocorrencia1"] # Left Join 1
cruzamento2 <- cruzamento1[aeronave, on = "codigo_ocorrencia2"] # Left Join 2
cruzamento3 <- cruzamento2[fator_contribuinte, on = "codigo_ocorrencia3"] # Left Join 3
tabela <- cruzamento3[recomendacao, on = "codigo_ocorrencia4"] # Left Join 4 - dá erro por chave deixar de ser única

E deu erro de novo! Terei que morder minha língua aqui. Analisando um pouco melhor, após cada cruzamento, ele está duplicando algumas chaves - tornando bem difícil fazer um *left join* ou um *right join* convencional.

Vamos adentrar por outro caminho. Sabemos que a tabela de recomendações possui apenas informações complementares e que não há problema (no nosso caso) de repetir as informações para cada código de ocorrência. Portanto, ao invés de fazer um *left join* como os outros, vamos partir para um *cartesian join* - já que apenas precisamos trazer essa informação para código de ocorrência, não importando se irá repetir ou não.

```{r Cruzando as tabelas finalmente}
cruzamento1 <- ocorrencia[ocorrencia_tipo, on = "codigo_ocorrencia1"] # Left Join 1
cruzamento2 <- cruzamento1[aeronave, on = "codigo_ocorrencia2"] # Left Join 2
cruzamento3 <- cruzamento2[fator_contribuinte, on = "codigo_ocorrencia3"] # Left Join 3
tabela <- cruzamento3[recomendacao, on = "codigo_ocorrencia4", allow.cartesian = TRUE] # Cartesian Join 1

head(tabela)
```

E deu certo! *Wheeew.* Agora que finalmente temos uma base com todos os dados agregados, podemos brincar um pouco com ela! :)
